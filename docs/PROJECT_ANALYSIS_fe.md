...
Dev server trên port 5175; proxy /api tới backend http://localhost:5000 để tránh CORS.
Ứng dụng:
...
Server:
src/server.js: khởi động trên PORT (mặc định 5000), kiểm tra DB, load models/associations, tùy chọn sequelize.sync({ alter: true }) trong dev nếu DB*SYNC=true.
...
Swagger:
src/config/swagger.js: OpenAPI 3, server mặc định http://localhost:5000, bearer JWT, quét routes/**/\*.js và models/**/*.js.
...
Cách chạy BE:
Env cần thiết (ví dụ): DB*HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD, NODE_ENV=development, DB_SYNC=true (nếu muốn auto alter).
Dev: cd be && pnpm i && pnpm dev (mặc định: http://localhost:5000)
Swagger: mở http://localhost:5000/api-docs
Tích hợp FE ↔ BE
FE gọi API qua /api/* → Vite proxy tới http://localhost:5000/api/_.
BE bật CORS cho http://localhost:5175 nên tương thích với FE port 5175.
Static uploads phục vụ tại http://localhost:5000/uploads/_.
Điểm mạnh
